...
class Command(object):
    ...
    def do_get_projects(self, name):
        projects = self.backend.get_projects()
        print(projects)

    def do_create_category(self, project, name):
        category = self.backend.create_category(project, name)

    def do_get_category(self, project, name):
        category = self.backend.get_category(project, name)
        print(category)

    def do_get_categories(self, project):
        categories = self.backend.get_categories(project)
        print(categories)

    def do_create_task(self, project, category, name):
        task = self.backend.create_task(project, category, name)
